...
//   When starting with . (clean node), weaken it and turn right.
//   When starting with W (weakened node), infect it.
//   When starting with # (infected node), flag it for cleaning and turn left.
//   When starting with F (flagged node), clean it and reverse direction.
//   Change the state of the node, then move one step in the direction the carrier is facing.
//
// Input:
...
//
// Output:
//   the number of infections that were caused in 10000000 turns
//
...
for (i=0; i<maxBursts; i++) {
  ...
  switch (curLine.charAt(vPos)) {
    case clean:  // landed on a clean node: left turn, weaken
      ...
      // weaken the node
      line[hPos] = changeBit(curLine,vPos,weakened);
      break;

    case unclean:  // landed on an infected node: right turn, flag
      ...
      // flag the node
      line[hPos] = changeBit(curLine,vPos,flagged);
      break;

    case weakened:  // landed on a weakened node: no turn, infect
    ...
    case flagged:  // landed on a flagged node: reverse direction, clean
